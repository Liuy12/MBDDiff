...
```{r,message = FALSE, warning=FALSE, eval=FALSE}
MAplot(TestStat[[2]])
DE_inex <- with(Testres, which(baseMean > quantile(baseMean)[2] & 
...
MBDDiff also generate a dynamic html report containing all the figures above:

```{r,message = FALSE, warning=FALSE, eval=FALSE}
MBDDynamicReport(bin_count, fa, MBD, TestStat, )
```
